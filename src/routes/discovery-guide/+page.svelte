<script lang="ts">
    ...
    const copy = {
        en: {
            ...
            experiences: 'Four ways to listen', modes: [
                ...
                ['Music Docuseries', 'Follow documentary-style series that bring music history, people, movements, and memorable moments into a guided listening journey.']
            ],
            libraries: 'Browse and understand the libraries', libraryText: 'The libraries offer different paths into discovery: Artist Spotlight for artists and their stories, the Nostalgia Library for ranked decade-and-genre programs, and the Collection Library for curated musical themes and traditions.',
            multilingual: 'Music discovery and storytelling in three languages', multilingualText: 'TopSpot40 is available in English, Spanish, and Portuguese so that music history, artist stories, and guided discovery can be enjoyed in the listener’s preferred language.',
        ...
        ptbr: {
            ...
            experiences: 'Quatro maneiras de ouvir', modes: [
                ...
                ['Docusséries musicais', 'Acompanhe séries em estilo documental que trazem a história da música, pessoas, movimentos e momentos marcantes para uma jornada de audição guiada.']
            ],
            libraries: 'Navegue e entenda as bibliotecas', libraryText: 'As bibliotecas oferecem caminhos diferentes para a descoberta: Destaques de artistas para os artistas e suas histórias, a Biblioteca de nostalgia para programas classificados por década e gênero, e a Biblioteca de coleções para temas e tradições musicais selecionados.',
            multilingual: 'Descoberta musical e narrativas em três idiomas', multilingualText: 'O TopSpot40 está disponível em inglês, espanhol e português para que a história da música, as histórias dos artistas e a descoberta guiada possam ser apreciadas no idioma preferido de cada ouvinte.',
            closing: 'Um convite à descoberta', closingText: 'O TopSpot40 é construído em torno da curiosidade: outro artista para conhecer, outra história para ouvir, outra cultura para apreciar e outra música esperando para se tornar favorita.', open: 'Leia mais', close: 'Mostrar menos'
        }
    } as const;
...
{#if hasLanguage}
    <div class="info-page">
        ...
        <main>
            ...
            <section class="closing" aria-labelledby="closing-title"><h2 id="closing-title">{copy[language].closing}</h2><p>{copy[language].closingText}</p><a class="back bottom" href="/welcome">← {copy[language].back}</a></section>
        </main>
    </div>
{/if}
