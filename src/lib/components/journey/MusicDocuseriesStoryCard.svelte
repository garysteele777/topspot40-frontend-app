...
<a class="story-card" {href}>
    <span class="artwork">
        <img class:fallback={!story.artwork_url || imageFailed}
             src={!imageFailed && story.artwork_url ? story.artwork_url : MUSIC_DOCUSERIES_FALLBACK_ARTWORK} alt=""
...
<style>
    .story-card {
        ...
        overflow: hidden;
        color: #fff;
        background: linear-gradient(135deg, rgba(43, 38, 29, .98), rgba(20, 20, 19, .98));
        border: 1px solid rgba(215, 166, 74, .36);
        border-radius: 18px;
    ...
    .shade {
        position: absolute;
        inset: 0;
        background: linear-gradient(135deg, transparent, rgba(9, 6, 2, .68));
    }
