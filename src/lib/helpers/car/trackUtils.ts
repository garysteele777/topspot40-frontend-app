...
export function pickInitialTrack(
    ...
    const findFirstUnplayed = (tracks: LoadedTrack[]) =>
        tracks.find(t => !playedRanks.has(t.rank)) ?? null;

    // 🎯 SHUFFLE
    if (order === 'shuffle') {
        const shuffled = [...list].sort(() => Math.random() - 0.5);
        return findFirstUnplayed(shuffled) ?? shuffled[0];
    }
    ...
    for (const t of sorted) {
        if (t.rank >= startRank && !playedRanks.has(t.rank)) {
            return t;
        }
    }
